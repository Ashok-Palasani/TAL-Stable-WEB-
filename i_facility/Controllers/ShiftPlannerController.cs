...
namespace i_facility.Controllers
{
    public class ShiftPlannerController : Controller
    {
        ...
        public JsonResult GetWC_Cell_MWCforole(int CellID)
        {
            var MachineData = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.CellID == CellID && m.ManualWCID != null), "MachineID", "MachineInvNo");
            return Json(MachineData, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetShopForSp(int plantId)
        {
            List<tblshop> shoplist = new List<tblshop>();
            var shopdet = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.IsNormalWC == 2).Select(m => m.ShopID).Distinct().ToList();
            foreach (var macrow in shopdet)
            {
                tblshop psc = new tblshop();
                var shopdata = db.tblshops.Where(m => m.IsDeleted == 0 && m.ShopID == macrow && m.PlantID == plantId).ToList();
                foreach (var shop in shopdata)
                {
                    psc.ShopID = shop.ShopID;
                    psc.ShopName = shop.ShopName;
                    shoplist.Add(psc);
                }
            }
        ...
        public JsonResult GetCellForSp(int shopId)
        {
            List<tblcell> celllist = new List<tblcell>();
            var celldet = db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.IsNormalWC == 2).Select(m => m.CellID).Distinct().ToList();
            foreach (var macrow in celldet)
            {
                tblcell psc = new tblcell();
                var celldata = db.tblcells.Where(m => m.IsDeleted == 0 && m.CellID == macrow && m.ShopID == shopId).ToList();
                foreach (var cellrow in celldata)
                {
                    psc.CellID = cellrow.CellID;
                    psc.CellName = cellrow.CellName;
                    celllist.Add(psc);
                }
            }
            return Json(celllist, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetMachineForSp(int cellId)
        {
            var MachineData = new SelectList(db.tblmachinedetails.Where(m => m.IsDeleted == 0 && m.CellID == cellId && m.IsNormalWC == 2), "MachineID", "MachineInvNo");
            return Json(MachineData, JsonRequestBehavior.AllowGet);
        }
    }
}
