...
namespace i_facility.Controllers
{
    public class HMIScreeController : Controller
    {
...

        {
            ...
            string workcodeno = null;


            if (MacInvNo == null)
            {
                //var workcode = obj.GetMachineDet21(workcntrcode, Machineid);
                //foreach (var row in workcode)
                //{
                //    string workcodeno = Convert.ToString(row.workcntrcode);
        ...
        public ActionResult DDLList(string data1, string selectedValue = "")
        {
            ...
            if (WOCount == 1)
            {
                ...
                if (isValid)
                {
                    ...
                    // hmidata.HMIID = ;
                    obj.InsertLiveHMIDetailsddl(DateTime.Now, ddldata.OperationNo, ddldata.PartName, ddldata.Project, CorrectedDate, Shift, ddldata.Type, Convert.ToInt32(Session["UserID"]), OpName, Convert.ToInt32(ddldata.TargetQty), ddldata.WorkOrder, hmidata.isWorkOrder, TotalProcessQty, ishold, ddldata.WorkCenter, machineId);
        ...
        public bool CheckAlltheWO(string woNo, string opNo, string partNo, out string OperationNum)
        {
            ...
            var ddldet = obj.GettblddlWOList(woNo, partNo, opNo);
